...
import static it.eng.parer.fascicolo.runner.util.EndPointCostants.URL_GET_INFOS;
...
class InfosEndpointTest {
    ...
    void successAny() {
        given().when().get(URL_GET_INFOS).then().statusCode(200).body("$", hasKey("git")).body("git.git.build.version",
                not(empty()));
    }
