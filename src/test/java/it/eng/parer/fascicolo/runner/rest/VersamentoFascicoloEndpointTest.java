...
class VersamentoFascicoloEndpointTest {
    ...
    void wrongXmlRequest_fails() {
        given().config(RestAssured.config()
                ...
                .header(HttpHeaders.CONTENT_TYPE, MediaType.MULTIPART_FORM_DATA).urlEncodingEnabled(true)
                .multiPart("LOGINNAME", "fake", MediaType.TEXT_PLAIN)
                .multiPart("VERSIONE", "9999", MediaType.TEXT_PLAIN).multiPart("PASSWORD", "fake", MediaType.TEXT_PLAIN)
                .multiPart("XMLSIP", "<IndiceSIPFascicolo></IndiceSIPFascicolo>", MediaType.TEXT_XML).when()
                .post(URL_FASCICOLO_BASE + URL_PUBLIC_FASCICOLO_V3).then().statusCode(400)
                .body("EsitoVersamentoFascicolo.EsitoVersamentoNegativo.EsitoGenerale.CodiceEsito", is("NEGATIVO"))
    ...
    void missingParams_fails() {
        given().config(RestAssured.config()
                .encoderConfig(encoderConfig().encodeContentTypeAs("multipart/form-data", ContentType.TEXT)))
                .header(HttpHeaders.CONTENT_TYPE, MediaType.MULTIPART_FORM_DATA).urlEncodingEnabled(true).when()
                .post(URL_FASCICOLO_BASE + URL_PUBLIC_FASCICOLO_V3).then().statusCode(400)
                .body("EsitoVersamentoFascicolo.EsitoVersamentoNegativo.EsitoGenerale.CodiceEsito", is("NEGATIVO"))
                .body("EsitoVersamentoFascicolo.EsitoVersamentoNegativo.EsitoGenerale.CodiceErrore", is(WS_CHECK));
    }
    ...
    void wrongCredentials_fails() {
        given().config(RestAssured.config()
                .encoderConfig(encoderConfig().encodeContentTypeAs("multipart/form-data", ContentType.TEXT)))
                .header(HttpHeaders.CONTENT_TYPE, MediaType.MULTIPART_FORM_DATA).urlEncodingEnabled(true)
                .multiPart("LOGINNAME", "not_exists", MediaType.TEXT_PLAIN)
