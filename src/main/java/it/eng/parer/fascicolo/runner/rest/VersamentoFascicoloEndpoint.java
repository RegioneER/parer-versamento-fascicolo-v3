...
public class VersamentoFascicoloEndpoint {
    ...
    @Inject
    public VersamentoFascicoloEndpoint(IVersFascicoloService versFascicoloService, SecurityContext securityCtx,
    ...
    public Response oauth2fascicolov3(@Valid VersamentoFascicoloOauth2MultipartForm formData) {
        ...
        return Response.ok(result)
                .lastModified(Date.from(result.getDataEsitoVersamentoFascicolo().toGregorianCalendar().toInstant()))
        ...
        try {
            ...
            if (rispostaWSFascicolo.getSeverity() == IRispostaWS.SeverityEnum.OK) {
                ...
                if (rispostaWSFascicolo.getSeverity() == SeverityEnum.OK) {
                    ...
                    rispostaWSFascicolo.setStatoSessioneVersamento(IRispostaWS.StatiSessioneVersEnum.ERRATA);
                    //
                ...
                if (rispostaWSFascicolo.getSeverity() == SeverityEnum.OK) {
                    ...
                    versFascicoloService.verificaVersione(blockingFakeSession.getVersioneWS(), rispostaWSFascicolo,
                            versFascicoloExt);
                }

                /*
                 * Logica condivisa tra verifica "standard" e con token (OAut2+...futuri improvements)
                 */
                if (rispostaWSFascicolo.getSeverity() == SeverityEnum.OK) {
                ...
                if (rispostaWSFascicolo.getSeverity() == SeverityEnum.OK) {
                    ...
                    versFascicoloService.parseXML(blockingFakeSession, rispostaWSFascicolo, versFascicoloExt);
                }

                blockingFakeSession.setTmChiusura(ZonedDateTime.now());
                versFascicoloService.salvaTutto(blockingFakeSession, rispostaWSFascicolo, versFascicoloExt);

            }

            long totalTime = Duration.between(start, LocalDateTime.now()).toMillis();
            avanzamentoWs.setCheckPoint(AvanzamentoWs.CheckPoints.FINE).setFase("").setTotalTime(totalTime)
                    .logAvanzamento(true);
            return myEsito.produciEsitoFascicolo();
        } catch (Exception e) {
            throw new AppGenericRuntimeException(
                    "Errore generico in fase di versamento VersamentoFascicoloEndpoint.doVersamento", e,
