...
/*
 * ExceptionMapper che gestisce gli errori di tipoo WebApplicationException, nel caso specifico si deve verificare la logica
 * del provider CustomJaxbMessageBodyReader il quale, come da default, utilizza questo tipo di eccezioni.
 */
@Provider
public class AppGenericRuntimeExceptionMapperProvider implements ExceptionMapper<AppGenericRuntimeException> {

    private static final Logger log = LoggerFactory.getLogger(AppGenericRuntimeExceptionMapperProvider.class);
    ...
    public Response toResponse(AppGenericRuntimeException exception) {
        ...
        if (rispostaWSFascicolo.getSeverity() == IRispostaWS.SeverityEnum.ERROR) {
            versFascicoloExt.listErrAddError(StringUtils.EMPTY, myEsito.getEsitoGenerale().getCodiceErrore(),
        ...
        if (exception.getCause() instanceof XmlSipNotWellFormedException) {
            ...
            //
            versFascicoloExt.listErrAddError(StringUtils.EMPTY, MessaggiWSBundle.FAS_XSD_001_001, MessaggiWSBundle
                    .getString(MessaggiWSBundle.FAS_XSD_001_001, ExceptionUtils.getRootCauseMessage(exception)));
            ...
            rispostaWSFascicolo.setStatoSessioneVersamento(IRispostaWS.StatiSessioneVersEnum.DUBBIA);
        } else if (exception.getCause() instanceof XmlSipUnmarshalException) {
            // status
            status = Response.Status.BAD_REQUEST;
            //
            versFascicoloExt.listErrAddError(StringUtils.EMPTY, MessaggiWSBundle.FAS_XSD_001_002, MessaggiWSBundle
                    .getString(MessaggiWSBundle.FAS_XSD_001_002, ExceptionUtils.getRootCauseMessage(exception)));
            ...
            // default
            versFascicoloExt.listErrAddError(StringUtils.EMPTY, MessaggiWSBundle.ERR_666, MessaggiWSBundle
                    .getString(MessaggiWSBundle.ERR_666, ExceptionUtils.getRootCauseMessage(exception)));
        ...
        rispostaWSFascicolo
                .setErrorMessage(MessaggiWSBundle.getString(tmpVdE.getErrorCode(), tmpVdE.getErrorMessage()));
        ...
        versFascicoloExt.getStrutturaComponenti().setDataVersamento(
                XmlDateUtility.xmlGregorianCalendarToDateOrNull(myEsito.getDataRapportoVersamento()));
        ...
        avanzamentoWs.setCheckPoint(AvanzamentoWs.CheckPoints.VERIFICA_XML).setFase("completata").logAvanzamento();
