...
/*
 * ExceptionMapper delegato alla raccolta degli errori di tipo ConstraintViolationException che si possono verificare
 * a vari livelli (validazione form, chiamata a metodo mancante parametri previsti, ecc.).
 */
@Provider
public class ConstraintViolationExceptionMapperProvider implements ExceptionMapper<ConstraintViolationException> {

    private static final Logger log = LoggerFactory.getLogger(ConstraintViolationExceptionMapperProvider.class);
    ...
    public Response toResponse(ConstraintViolationException exception) {
        // log
        log.atError().log("ConstraintViolationExceptionMapperProvider errore registrato", exception);
        // init
        ...
        exception.getConstraintViolations().forEach(c -> {
            if (c.getLeafBean() instanceof IVersamentoFascicoloMultipartForm) {
                versFascicoloExt.listErrAddError(StringUtils.EMPTY, MessaggiWSBundle.WS_CHECK, c.getMessage());
            } else {
                // default
                versFascicoloExt.listErrAddError(StringUtils.EMPTY, MessaggiWSBundle.ERR_666, c.getMessage());
            }
        });
        ...
        avanzamentoWs.setCheckPoint(AvanzamentoWs.CheckPoints.VERIFICA_STRUTTURA_CHIAMATA_WS).setFase("completata")
                .logAvanzamento();

        // salva sessione
        versFascicoloSync.salvaTutto(blockingFakeSession, rispostaWSFascicolo, versFascicoloExt);

        return Response.status(Response.Status.BAD_REQUEST).entity(myEsito.produciEsitoFascicolo()).build();
    }
