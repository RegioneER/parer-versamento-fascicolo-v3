...
public class WSLoginHandler {
     ...
     */
    public boolean checkAuthz(String username, Integer idOrganiz, String servizioWeb) throws AuthWSException {
    ...
    /**
     * Controllo di autenticazione per un utente tramite la password / IP In caso di autenticazione con token (OAuth2)
     * viene estratto lo userid + controllo (eventuale) di IP
     *
     * @param username
     *            nome utente
     * @param password
     ...
     *             eccezione lanciata se l'utente non è autorizzato
     */
    public boolean login(String username, String password, String ipAddress) throws AuthWSException {
    ...
    private boolean doLogin(String username, String password, String ipAddress, Query q1, Query ipListQuery)
            throws AuthWSException {
        Object[] res;
        try {
        ...
        if (res != null) {
            ...
            if (LocalDateTime.now().isAfter(expDate)) {
                throw new AuthWSException(AuthWSException.CodiceErrore.UTENTE_SCADUTO,
                        "Credenziali scadute in data " + expDate.format(DateTimeFormatter.ofPattern("dd/MM/yyyy")));
            }
