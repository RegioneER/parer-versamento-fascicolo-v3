...
public class VersFascicoloService implements IVersFascicoloService {
    ...
    public AvanzamentoWs init(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento) {
        ...
        // load versions
        RispostaControlli rcLoadWsVers = controlliWsService.loadWsVersions(versamento.getDescrizione());
        // if positive ...
        ...
        //
        myEsitoComposito.setVersioneRapportoVersamento(versamento.getDescrizione().getVersione(wsVersions));
        //
        ...
        if (!rcLoadWsVers.isrBoolean()) {
            ...
            myEsitoComposito.getEsitoChiamataWS().setCredenzialiOperatore(ECEsitoPosNegNesType.NON_ESEGUITO);
            myEsitoComposito.getEsitoChiamataWS().setVersioneWSCorretta(ECEsitoPosNegNesType.NON_ESEGUITO);
        }
    ...
    public void verificaCredenziali(String loginName, String password, String indirizzoIp,
        ...
        CompRapportoVersFascicolo myEsito = rispostaWs.getCompRapportoVersFascicolo();
        User tmpUser = null;
        RispostaControlli rcCheckCred = controlliWsService.checkCredenziali(loginName, password, indirizzoIp);
    ...
    @Override
    public void parseXML(BlockingFakeSession sessione, RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento) {
        ...
        try {
            ...
            if (ExceptionUtils.getRootCause(e) instanceof ParamApplicNotFoundException) {
                ...
                rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666,
                        "Errore generico nella fase di parsing dell'XML " + ExceptionUtils.getMessage(e));
            }
    ...
    @Override
    public void salvaTutto(BlockingFakeSession sessione, RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento) {
        ...
        if (!versamento.isSimulaScrittura()
            ...
            if (rispostaWs.getSeverity() == IRispostaWS.SeverityEnum.ERROR) {
                if (rispostaWs.getStatoSessioneVersamento() == IRispostaWS.StatiSessioneVersEnum.DUBBIA) {
                    ...
                    // errata)
                    recupSessDubbieFascService.recuperaSessioneErrata(rispostaWs, versamento, sessione);
                }
                // salva sessione
                if (rispostaWs.getStatoSessioneVersamento() == IRispostaWS.StatiSessioneVersEnum.FALLITA) {
                    ...
                    myEsito.setWarningUlteriori(versamento.produciEsitoWarningSec());
                    try {
                        logSessioneFascicoliService.registraSessioneFallita(rispostaWs, versamento, sessione);
                    } catch (AppGenericPersistenceException agpe) {
                        log.atError().log("Errore interno nella fase di salvataggio sessione fallita.", agpe);
                ...
                if (rispostaWs.getStatoSessioneVersamento() == IRispostaWS.StatiSessioneVersEnum.ERRATA) {
                    ...
                    myEsito.setWarningUlteriori(versamento.produciEsitoWarningSec());
                    try {
                        logSessioneFascicoliService.registraSessioneErrata(rispostaWs, versamento, sessione);
                    } catch (AppGenericPersistenceException agpe) {
                        log.atError().log("Errore interno nella fase di salvataggio sessione errata.", agpe);
                    }
                }
