...
public class SalvataggioBackendDao implements ISalvataggioBackendDao {
    ...
    public String getBackendSessioniErrKo() throws ObjectStorageException {
        try {
            return configurationDao.getValoreParamApplicByApplic(ParametroApplDB.BACKEND_XML_SES_FASC_ERR_KO);

        } catch (ParamApplicNotFoundException | IllegalArgumentException e) {
            throw ObjectStorageException.builder().message(NO_PARAMETER, ParametroApplDB.BACKEND_XML_SES_FASC_ERR_KO)
                    .cause(e).build();
        }
    }
     ...
     *            id della tipologia dell'anno del fascicolo
     * @param nomeWs
     *            nome del servizio
     *
     * @return configurazione del backend. Può essere, per esempio OBJECT_STORAGE_STAGING oppure DATABASE_PRIMARIO
     ...
     *             in caso di errore
     */
    public String getBackendByServiceName(long idAaTipoFascicolo, String nomeWs) throws ObjectStorageException {
        try {
            ...
            throw ObjectStorageException.builder()
                    .message("Impossibile ottenere il parametro per id tipo anno fascicolo {0} e nome servizio {1}",
                            idAaTipoFascicolo, nomeWs)
                    .cause(e).build();
        }
    }

    private String getParameter(String parameterName, long idAaTipoFascicolo) {
        DecAaTipoFascicolo aaTipoFascicolo = entityManager.find(DecAaTipoFascicolo.class, idAaTipoFascicolo);
        long idStrut = aaTipoFascicolo.getDecTipoFascicolo().getOrgStrut().getIdStrut();

        long idAmbiente = aaTipoFascicolo.getDecTipoFascicolo().getOrgStrut().getOrgEnte().getOrgAmbiente()
                .getIdAmbiente();

        return configurationDao.getValoreParamApplicByAaTipoFasc(parameterName, idStrut, idAmbiente, idAaTipoFascicolo);
    }
     ...
     * @param base64crc32
     *            eventuale base64-encoded CRC32C del file per data integrity check
     *
     * @return riferimento alla risorsa appena inserita
     *
     * @throws ObjectStorageException
    ...
    public ObjectStorageResource putObject(InputStream blob, long blobLength, final String key,
            ...
            Optional<String> base64crc32) throws ObjectStorageException {
    ...
    /**
     * Effettua il salvataggio del collegamento tra i sip del fascicolo e la chiave sull'object storage
     *
     * @param object
     ...
     *             in caso di errore
     */
    public void saveObjectStorageLinkSipFasc(ObjectStorageResource object, String nmBackend, long idFascicolo,
    ...
    /**
     * Effettua il salvataggio del collegamento tra i metadati (profili) del fascicolo e la chiave sull'object storage
     *
     * @param object
     *            informazioni dell'oggetto salvato
     * @param nmBackend
     ...
     *             in caso di errore
     */
    public void saveObjectStorageLinkMetaProfFasc(ObjectStorageResource object, String nmBackend, long idFascicolo,
            BigDecimal idStrut) throws ObjectStorageException {
        try {
    ...
    public void saveObjectStorageLinkSipSessioneKo(ObjectStorageResource object, String nmBackend,
        ...
        try {
            ...
            VrsSesFascicoloKo sesFascicoloKo = entityManager.find(VrsSesFascicoloKo.class, idSesFascicoloKo);
    ...
    public void saveObjectStorageLinkSipSessioneErr(ObjectStorageResource object, String nmBackend,
        ...
        try {
            ...
            VrsSesFascicoloErr sesFascicoloErr = entityManager.find(VrsSesFascicoloErr.class, idSesFascicoloErr);
     ...
     * @param tipoUsoOs
     *            ambito di utilizzo di questo backend (per esempio METADATI_FASCICOLI)
     *
    ...
    public ObjectStorageBackend getObjectStorageConfiguration(final String nomeBackend, final String tipoUsoOs)
        ...
        final URI osURI;
        final String osBucket;

        if (s3Clients.isClientProvided()) {

            Optional<String> bucket = null;

            switch (tipoUsoOs) {
            case METADATI_FASCICOLI:
                bucket = bucketNameMetadataFasc;
                break;
            case SESSIONI_FASC_ERR_KO:
                bucket = bucketNameSesErrKoFasc;
                break;
            default:
                throw ObjectStorageException.builder()
                        .message("Impossibile stabilire la tipologia di utilizzo per l'object storage").build();
            }
            if (bucket.isEmpty() || staticProviderAccessKeyId.isEmpty() || staticProviderSecretAccessKey.isEmpty()) {
                throw ObjectStorageException.builder()
                        .message("Impossibile stabilire la tipologia del parametro per l'object storage").build();
            }

            accessKeyId = staticProviderAccessKeyId.get();
            secretKey = staticProviderSecretAccessKey.get();
            osURI = s3Clients.getStaticProviderClient().serviceClientConfiguration().endpointOverride().get();
            osBucket = bucket.get();
            ...
            Optional<String> nomeEnvironmentVariableAccessKeyId = null;
            Optional<String> nomeEnvironmentVariableSecretKey = null;

            switch (tipoUsoOs) {
            case METADATI_FASCICOLI:
                nomeEnvironmentVariableAccessKeyId = accessKeyIdMetaFasc;
                nomeEnvironmentVariableSecretKey = secretKeyMetaFasc;
                break;
            case SESSIONI_FASC_ERR_KO:
                nomeEnvironmentVariableAccessKeyId = accessKeyIdSesErrKoFasc;
                nomeEnvironmentVariableSecretKey = secretKeySesErrKoFasc;
                break;
            default:
                throw ObjectStorageException.builder()
                        .message("Impossibile stabilire la tipologia di utilizzo per l'object storage").build();
            }
            if (nomeEnvironmentVariableAccessKeyId.isEmpty() || nomeEnvironmentVariableSecretKey.isEmpty()) {
                throw ObjectStorageException.builder()
                        .message("Impossibile stabilire la tipologia del parametro per l'object storage").build();
            }

            accessKeyId = nomeEnvironmentVariableAccessKeyId.get();
            secretKey = nomeEnvironmentVariableSecretKey.get();
            osURI = URI.create(storageAddress);
            osBucket = bucket;
        }
     ...
     *
     * @param idFascicolo
     *            identificativo del fascicolo di cui salvare il contenuto
     *
    ...
    public String generateKeyFascicolo(long idFascicolo) throws ObjectStorageException {
        try {
            ...
            throw ObjectStorageException.builder().message("Impossibile generare la chiave del fascicolo").cause(e)
                    .build();
        }
    }
    ...
    private String createKeyFascicoli(String nmEnteNorm, String nmStrutNorm, int anno, long idFascicolo) {
        // Non serve a nulla
