...
public class ProfileGenDataPrsr {
    ...
    private static DXPGEvento recuperaEvento(final Node tmpnode) throws DOMException, ParseException {
    ...
    private static DXPGIdentSoggetto recuperaFixedIdentSogPG(final String tagname, final Node parentNode) {
        DXPGIdentSoggetto tmpIdent = null;
        //
        NodeList tmplist = ((Element) parentNode).getElementsByTagName(tagname);
        if (tmplist.getLength() > 0 && StringUtils.isNotBlank(tmplist.item(0).getTextContent()) /* skip if empty */) {
    ...
    private static DXPGSoggetto recuperaSoggettoPG(final Node parentNode) throws DOMException, ParseException {
        NodeList tmpList;
        boolean continua = true;
        // soggetto elaborato
    ...
    private static void elabTiPFOnSoggettoPG(final String tagName, final Node parentNode, DXPGSoggetto tmpSoggetto) {
        // get by tagname
    ...
    private static void elabIndirizziDigitaliDiRiferimento(Node localNodeSog, DXPGSoggetto tmpSoggetto) {
        NodeList nodesIndirRif = ((Element) localNodeSog).getElementsByTagName("IndirizziDigitaliDiRiferimento");
        if (nodesIndirRif.getLength() > 0) {
            ...
            if (localNodeIndirRif.getNodeType() == Node.ELEMENT_NODE) {
                ...
                NodeList tmpList = ((Element) localNodeIndirRif).getElementsByTagName("IndirizzoDigitaleDiRiferimento");
                for (int i = 0; i < tmpList.getLength(); i++) {
                    Node localtmpnode = tmpList.item(i);
                    if (localtmpnode.getNodeType() == Node.ELEMENT_NODE) {
    ...
    private static void elabIdentificativi(Node localNodeSog, DXPGSoggetto tmpSoggetto) {
        ...
        if (nodesIdent.getLength() > 0) {
            ...
            if (localNodeIdent.getNodeType() == Node.ELEMENT_NODE) {
                // Identificativo
                NodeList tmpList = ((Element) localNodeIdent).getElementsByTagName("Identificativo");
                for (int i = 0; i < tmpList.getLength(); i++) {
                    Node localtmpnode = tmpList.item(i);
                    if (localtmpnode.getNodeType() == Node.ELEMENT_NODE) {
                        tmpSoggetto.addIdentificativo(recuperaGenericIdentSogPG(localtmpnode));
                        tmpSoggetto.addIdentificativo(recuperaFixedIdentSogPG("IPAAmm", localtmpnode));
                        tmpSoggetto.addIdentificativo(recuperaFixedIdentSogPG("IPAAOO", localtmpnode));
                        tmpSoggetto.addIdentificativo(recuperaFixedIdentSogPG("IPAUOR", localtmpnode));
                    }
                }
            }
        }
