...
public class LogSessioneFascicoliService implements ILogSessioneFascicoliService {
    ...
    public void registraSessioneErrata(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
        ...
        try {
            ...
            rcSessErr = logSessioneFascicoliDao.scriviFascicoloErr(rispostaWs, versamento, sessione);

            // salvo gli xml di request & response
            tmpSesFascicoloErr = (VrsSesFascicoloErr) rcSessErr.getrObject();
            //
            logSessioneFascicoliDao.scriviXmlFascicoloErr(rispostaWs, versamento, tmpSesFascicoloErr, backendMetadata,
                    sipBlob);
            //
            // MEV#30786
            /*
            ...
            if (backendMetadata.isObjectStorage()) {
                ObjectStorageResource res = objectStorageService.createSipInSessioniErr(
                        backendMetadata.getBackendName(), sipBlob, tmpSesFascicoloErr.getIdSesFascicoloErr(),
            ...
            // dati.
            rispostaWs.setSeverity(IRispostaWS.SeverityEnum.ERROR);
            rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P, msg + ExceptionUtils.getRootCauseMessage(ex));
            versamento.aggiungErroreFatale(rispostaWs.getCompRapportoVersFascicolo().getEsitoGenerale());
            // si scatena una AppGenericPersistenceException contenente
            // l'eccezione runtime non gestita dalla catch precedente
            throw new AppGenericPersistenceException(ex, msg + ExceptionUtils.getRootCauseMessage(ex));
        }
    }
        ...
        try {
            ...
            if (rispostaWs.isErroreElementoDoppio()) {
                ...
                if (rcSessFailed.getrObject() != null) {
                    ...
                    rcSessFailed = logSessioneFascicoliDao.scriviFascicoloKo(rispostaWs, versamento, sessione);
                    //
                    tmpFascicoloKo = (VrsFascicoloKo) rcSessFailed.getrObject();
                }
            }
            ...
            rcSessFailed = logSessioneFascicoliDao.scriviSessioneFascicoloKo(rispostaWs, versamento, sessione,
                    tmpFascicoloKo, tmpFasFascicolo);
            //
            tmpSesFascicoloKo = (VrsSesFascicoloKo) rcSessFailed.getrObject();
            // salvo xml di request & response
            logSessioneFascicoliDao.scriviXmlFascicoloKo(rispostaWs, versamento, sessione, tmpSesFascicoloKo,
                    backendMetadata, sipBlob);
             ...
             */
            if (backendMetadata.isObjectStorage()) {
                ObjectStorageResource res = objectStorageService.createSipInSessioniKo(backendMetadata.getBackendName(),
                        sipBlob, tmpSesFascicoloKo.getIdSesFascicoloKo(), getIdStrut(versamento));
                log.debug("Salvati i SIP nel bucket {} con chiave {} ", res.getBucket(), res.getKey());
            }
            // end MEV#30786
            entityManager.flush();
