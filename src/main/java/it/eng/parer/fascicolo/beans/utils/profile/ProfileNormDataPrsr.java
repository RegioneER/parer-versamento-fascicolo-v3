...
public class ProfileNormDataPrsr {
    ...
    /**
     * Il parsing dei dati provenienti dal profilo normativo prevede: - estrazione soggetti - definizione di eventi su
     * soggetti
     *
     * @param pnt
     *            profilo normativo da indice SIP
     *
        ...
        try {
            ...
            for (int i = 0; i < nodes.getLength(); i++) {
                ...
                if (tmpnode != null && tmpnode.getNodeType() == Node.ELEMENT_NODE) {
                    tmpDatiXml.addSoggetto(recuperaSoggettoPN("AmministrazionePartecipante", tmpnode));
                    tmpDatiXml.addSoggetto(recuperaSoggettoPN("AmministrazioneTitolare", tmpnode));
                    tmpDatiXml.addSoggetto(recuperaSoggettoPN("SoggettoIntestatarioPersonaFisica", tmpnode));
                    tmpDatiXml.addSoggetto(recuperaSoggettoPN("SoggettoIntestatarioPersonaGiuridica", tmpnode));
    ...
    private static void elabPAEOnSoggettoPN(final Node parentNode, DXPNSoggetto tmpSoggetto) {
        ...
        if (nodesPAE.getLength() > 0) {
            ...
            // can be null
            NodeList tmplist = ((Element) localtmpnode).getElementsByTagName("DenominazioneAmministrazione");
            if (tmplist.getLength() > 0) {
    ...
    private static DXPNSoggetto recuperaAssegnazione(final String tagName, final Node parentNode, int idx)
            throws DOMException {
        DXPNSoggetto tmpSoggetto = new DXPNSoggetto();
        boolean continua = false;
        ...
        if (nodesAss.getLength() > 0) {
            ...
            if (tmpnode != null && tmpnode.getNodeType() == Node.ELEMENT_NODE) {
                // can be null
                NodeList tmpList = ((Element) tmpnode).getElementsByTagName("TipoAssegnazioneRuolo");
                if (tmpList.getLength() > 0) {
                    continua = true;
                    tmpSoggetto.setTipoRapporto("SoggettoAssegnatario - " + tmpList.item(0).getTextContent());
    ...
    private static void elabSoggettoOnAssegnazione(final String tagName, final Node parentNode,
        ...
        if (tmplist.getLength() > 0) {
            ...
            if (localtmpnode != null && localtmpnode.getChildNodes() != null
                ...
                // default
                tmpSoggetto.setDenominazioneDefault(Costanti.SOGG_DENOMINAZIONE_DEFAULT
                        .concat(" - " + String.format(Costanti.UrnFormatter.PAD2DIGITS_FMT, (idx + 1))));
            }
        } // if
    }

    private static DXPNEvento elabEventoOnAssegnazione(final String desc, final Node parentNode) throws DOMException {
        NodeList tmpList;
        ...
        if (continua) {
            ...
            Date dataInzio = Date.from(LocalDateTime.parse(tmpStringDataInizio.toString(), formatter)
    ...
    private static void elabCodiceIPA(final String tagName, DXPNSoggetto tmpSoggetto, Node localtmpnode) {
    ...
    private static void elabIndirizziDigitaliDiRiferimento(Node localtmpnode, DXPNSoggetto tmpSoggetto) {
        NodeList tmplist = ((Element) localtmpnode).getElementsByTagName("IndirizziDigitaliDiRiferimento");
        if (tmplist.getLength() > 0) {
