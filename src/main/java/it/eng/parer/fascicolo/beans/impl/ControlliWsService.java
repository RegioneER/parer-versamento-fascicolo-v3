...
public class ControlliWsService implements IControlliWsService {
    ...
    public RispostaControlli checkVersione(String versione, String versioniWsKey, Map<String, String> xmlDefaults) {
        ...
        if (versione == null || versione.isEmpty()) {
            ...
            rcCheckVersione.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_003_001));
        ...
        if (versioniWs.isEmpty()) {
            ...
            rcCheckVersione.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_006_001, versioniWsKey));
            return rcCheckVersione;
        }

        // check if versione is supported
        rcCheckVersione.setrBoolean(versioniWs.contains(versione) && VersioneWS.issupported(versione));

        if (!rcCheckVersione.isrBoolean()) {
            rcCheckVersione.setCodErr(MessaggiWSBundle.FAS_CONFIG_003_002);
            rcCheckVersione.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_003_002, versione));
        }
    ...
    public RispostaControlli checkCredenziali(String loginName, String password, String indirizzoIP) {
        ...
        if (loginName == null || loginName.isEmpty()) {
            ...
            rcCheckCredenziali.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_002_007));
        ...
        try {
            ...
            final String queryStr = "select iu, usr from IamUser iu "
                    + "join UsrUser usr on iu.idUserIam = usr.idUserIam " + "where iu.nmUserid = :nmUseridIn";
            ...
            log.atWarn()
                    .log("ERRORE DI AUTENTICAZIONE WS." + " Funzionalità: {} Utente: {}"
                            ...
                            TipiWSPerControlli.VERSAMENTO_FASCICOLO.name(), loginName, e.getCodiceErrore().name(),
                            indirizzoIP, e.getDescrizioneErrore());
            ...
            if (e.getCodiceErrore().equals(AuthWSException.CodiceErrore.UTENTE_SCADUTO)) {
                ...
                rcCheckCredenziali.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_002_002, loginName));
                ...
                rcCheckCredenziali.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_002_004, loginName));
    ...
    public RispostaControlli checkAuthWS(User utente, IWSDesc descrizione) {
        ...
        try {
            ...
            rcCheckAuthWS
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_002_006, utente.getUsername()));
        } catch (Exception ex) {
            rcCheckAuthWS.setCodErr(MessaggiWSBundle.ERR_666);
            rcCheckAuthWS.setDsErr(
                    MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666, LOG_USER_VERIFY_ERR + ex.getMessage()));
        }

        if (checkOrgVersAuth) {
            try {
                final String queryStr = "select count(org) from IamAbilOrganiz org join org.iamUser usr where "
                        + "usr.idUserIam = :idUserIamIn " + "and org.idOrganizApplic = :idOrganizApplicIn";
                Query query = entityManager.createQuery(queryStr);
                ...
                if (numAbil > 0) {
                    ...
                    rcCheckAuthWS.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_002_005,
    ...
    public RispostaControlli checkAuthWSNoOrg(User utente, IWSDesc descrizione) {
        ...
        try {
            final String querString = "select count(iu) from IamUser iu " + "join iu.iamAbilOrganizs iao "
            ...
            if (num > 0) {
                ...
                rcCheckAuthWSNoOrg.setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.FAS_CONFIG_002_005,
                        utente.getUsername(), descrizione.getNomeWs()));
            }
        } catch (Exception e) {
            rcCheckAuthWSNoOrg.setCodErr(MessaggiWSBundle.ERR_666);
            rcCheckAuthWSNoOrg
                    .setDsErr(MessaggiWSBundle.getString(MessaggiWSBundle.ERR_666, LOG_AUTH_ERR + e.getMessage()));
        }
    ...
    public RispostaControlli loadWsVersions(IWSDesc descrizione) {
        RispostaControlli rs = controlliSemanticiService.caricaDefaultDaDB(TipoParametroAppl.VERSIONI_WS);
        // if positive ...
        if (rs.isrBoolean()) {
