...
public class SalvataggioFascicoliDao implements ISalvataggioFascicoliDao {
    ...
    @Override
    public RispostaControlli scriviFascicolo(VersFascicoloExt versamento, BlockingFakeSession sessione,
        ...
        try {
            ...
            //
            tmpFascicolo.setTiConservazione(it.eng.parer.fascicolo.jpa.entity.constraint.FasFascicolo.TiConservazione
                    .valueOf(sessione.getIndiceSIPFascicolo().getParametri().getTipoConservazione().name()));
            // TODO IL sistema di migraziobe va gestito
            //
            tmpFascicolo.setFlForzaContrClassif(
                    svf.getFlControlliFasc().isFlForzaContrFlassif() ? CostantiDB.Flag.TRUE : CostantiDB.Flag.FALSE);
            tmpFascicolo.setFlForzaContrColleg(
                    svf.getFlControlliFasc().isFlForzaContrColleg() ? CostantiDB.Flag.TRUE : CostantiDB.Flag.FALSE);
            tmpFascicolo.setFlForzaContrNumero(
                    svf.getFlControlliFasc().isFlForzaContrNumero() ? CostantiDB.Flag.TRUE : CostantiDB.Flag.FALSE);
            ...
            if (StringUtils.isNotBlank(dxpa.getDescIndiceClassificazione())) {
                tmpFascicolo.setDsIndiceClassif(LogSessioneUtils.getStringAtMaxLen(dxpa.getDescIndiceClassificazione(),
                        Costanti.DS_INDICE_CLASS_MAX_LEN));
            }
            //
            ...
            if (svf.getDatiXmlProfiloGenerale() != null) {
                ...
                if (StringUtils.isNotBlank(dxpg.getOggettoFascicolo())) {
                    tmpFascicolo.setDsOggettoFascicolo(LogSessioneUtils.getStringAtMaxLen(dxpg.getOggettoFascicolo(),
                            Costanti.DS_OGGETTO_FASC_MAX_LEN));
                }
                // popolamento data apertura, chiusura, ud first e last, aaconservazione, note
                if (dxpg.getDataApertura() != null) {
                    tmpFascicolo.setDtApeFascicolo(convert(dxpg.getDataApertura()).toLocalDateTime());
                }
                if (dxpg.getDataChiusura() != null) {
                    tmpFascicolo.setDtChiuFascicolo(convert(dxpg.getDataChiusura()).toLocalDateTime());
                }
                if (StringUtils.isNotBlank(dxpg.getNoteFascicolo())) {
                    tmpFascicolo.setDsNota(
                            LogSessioneUtils.getStringAtMaxLen(dxpg.getNoteFascicolo(), Costanti.DS_NOTA_MAX_LEN));
                }
            ...
            if (fascicoloKo != null) {
                ...
                tmpFascicolo.setFlSesFascicoloKo(CostantiDB.Flag.FALSE); // il campo in tabella è NOT NULL
            }
            ...
            if (svf.getDatiXmlProfiloGenerale() != null) {
                for (DXPGEvento event : svf.getDatiXmlProfiloGenerale().getEventi()) {
                    ...
                    if (event.getDataInizio() != null) {
                        eventoFasciolo.setTsApertura(convert(event.getDataInizio()).toLocalDateTime());
                    }
                    if (event.getDataFine() != null) {
                        eventoFasciolo.setTsChiusura(convert(event.getDataFine()).toLocalDateTime());
                    }
            ...
            throw new AppGenericPersistenceException(e, LOG_FASCICOLO_ERR + ExceptionUtils.getRootCauseMessage(e));
        }
    ...
    public void scriviSogggetti(VersFascicoloExt versamento, FasFascicolo fascicolo)
        ...
        try {
            ...
            // generale
            if (svf.getDatiXmlProfiloGenerale() != null) {
                soggCount = manageSoggetti(fascicolo, svf.getDatiXmlProfiloGenerale().getSoggetti(), soggCount);
            }

            // normativo
            if (svf.getDatiXmlProfiloNormativo() != null) {
                manageSoggetti(fascicolo, svf.getDatiXmlProfiloNormativo().getSoggetti(), soggCount);
            }
            // OK
            entityManager.flush();
        } catch (Exception e) {
    ...
    private int manageSoggetti(FasFascicolo fascicolo, List<? extends ISoggetto> soggetti, int soggCount) {
        for (ISoggetto<IIdentSoggetto, IEvento> sogg : soggetti) {
            ...
            if (sogg.getIdentificativi().isEmpty()) {
                ...
                codIdeSog.setNmCodSog(generateSogFascLocalIdent(fascicolo.getAaFascicolo().toString(),
                ...
                for (IIdentSoggetto ident : sogg.getIdentificativi()) {
                    ...
                    //
                    codIdeSog.setFasSogFascicolo(fasSogFascicolo);
                    codIdeSog.setTiCodSog(ident.isPredefined() ? TiCodSog.PREDEFINITO : TiCodSog.NON_PREDEFINITO);
    ...
    @Override
    public void scriviRequestResponseFascicolo(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
            BlockingFakeSession sessione, BackendStorage backendMetadata, Map<String, String> sipBlob,
    ...
    public void scriviProfiliXMLFascicolo(VersFascicoloExt versamento, BlockingFakeSession sessione,
        ...
        try {
            ...
            String xmlProfilo = generaXmlProfilo(sessione.getIndiceSIPFascicolo().getProfiloGenerale().getAny(), true);
            fasXmlFascicolo.setFlCanonicalized(CostantiDB.Flag.TRUE);
            // MEV#30786
            if (backendMetadata.isDataBase()) {
                fasXmlFascicolo.setBlXml(xmlProfilo);
            } else {
                profBlob.put(TiModXsdFasXmlFascicolo.PROFILO_GENERALE_FASCICOLO.toString(), xmlProfilo);
            }
            // end MEV#30786
            ...
            if (svf.getIdRecXsdProfiloArchivistico() > 0) {
                ...
                xmlProfilo = generaXmlProfilo(sessione.getIndiceSIPFascicolo().getProfiloArchivistico().getAny(), true);
                fasXmlFascicolo.setFlCanonicalized(CostantiDB.Flag.TRUE);
                // MEV#30786
                if (backendMetadata.isDataBase()) {
                    fasXmlFascicolo.setBlXml(xmlProfilo);
                } else {
                    profBlob.put(TiModXsdFasXmlFascicolo.PROFILO_ARCHIVISTICO_FASCICOLO.toString(), xmlProfilo);
                }
                // end MEV#30786
                fasXmlFascicolo.setIdStrut(new BigDecimal(svf.getIdStruttura()));
                fasXmlFascicolo.setDtVersFascicolo(sessione.getTmApertura().toLocalDateTime());
            ...
            if (svf.getIdRecXsdProfiloNormativo() > 0) {
                fasXmlFascicolo = new FasXmlFascicolo();
                fasXmlFascicolo.setFasFascicolo(fascicolo);
                fasXmlFascicolo.setTiModelloXsd(TiModXsdFasXmlFascicolo.PROFILO_NORMATIVO_FASCICOLO);
                fasXmlFascicolo.setDecModelloXsdFascicolo(
                        entityManager.find(DecModelloXsdFascicolo.class, svf.getIdRecXsdProfiloNormativo()));
                xmlProfilo = generaXmlProfilo(
                        sessione.getIndiceSIPFascicolo().getProfiloNormativo().getValue().getAny(), true);
                fasXmlFascicolo.setFlCanonicalized(CostantiDB.Flag.TRUE);
                // MEV#30786
                if (backendMetadata.isDataBase()) {
                    fasXmlFascicolo.setBlXml(xmlProfilo);
                } else {
                    profBlob.put(TiModXsdFasXmlFascicolo.PROFILO_NORMATIVO_FASCICOLO.toString(), xmlProfilo);
                }
                // end MEV#30786
                fasXmlFascicolo.setIdStrut(new BigDecimal(svf.getIdStruttura()));
                fasXmlFascicolo.setDtVersFascicolo(sessione.getTmApertura().toLocalDateTime());
            ...
            if (svf.getIdRecXsdDatiSpec() > 0) {
                fasXmlFascicolo = new FasXmlFascicolo();
                fasXmlFascicolo.setFasFascicolo(fascicolo);
                fasXmlFascicolo.setTiModelloXsd(TiModXsdFasXmlFascicolo.PROFILO_SPECIFICO_FASCICOLO);
                fasXmlFascicolo.setDecModelloXsdFascicolo(
                        entityManager.find(DecModelloXsdFascicolo.class, svf.getIdRecXsdDatiSpec()));
                xmlProfilo = generaXmlProfiloSpec(sessione.getIndiceSIPFascicolo().getProfiloSpecifico());
                fasXmlFascicolo.setFlCanonicalized(CostantiDB.Flag.FALSE);
                // MEV#30786
                if (backendMetadata.isDataBase()) {
                    fasXmlFascicolo.setBlXml(xmlProfilo);
                } else {
                    profBlob.put(TiModXsdFasXmlFascicolo.PROFILO_SPECIFICO_FASCICOLO.toString(), xmlProfilo);
                }
                // end MEV#30786
                fasXmlFascicolo.setIdStrut(new BigDecimal(svf.getIdStruttura()));
                fasXmlFascicolo.setDtVersFascicolo(sessione.getTmApertura().toLocalDateTime());
            ...
            if (svf.getUnitaDocElencate() != null && !svf.getUnitaDocElencate().isEmpty()) {
                ...
                for (UnitaDocLink udColl : svf.getUnitaDocElencate()) {
                    ...
                    fasUnitaDocFascicolo
                            .setAroUnitaDoc(entityManager.find(AroUnitaDoc.class, udColl.getIdLinkUnitaDoc()));
                    if (udColl.getDataInserimentoFas() != null) {
                        fasUnitaDocFascicolo
                                .setDtDataInserimentoFas(convert(udColl.getDataInserimentoFas()).toLocalDateTime());
                    }
    ...
    @Override
    public void salvaWarningAATipoFascicolo(VersFascicoloExt versamento, BlockingFakeSession sessione)
            throws AppGenericPersistenceException {
        try {
            ...
            query.setParameter("aaTipoFascicolo",
                    new BigDecimal(sessione.getIndiceSIPFascicolo().getIntestazione().getChiave().getAnno()));
    ...
    public void scriviDatiSpecGen(VersFascicoloExt versamento, FasFascicolo tmpFasFascicolo)
        ...
        try {
            for (DatoSpecifico tmpDS : svf.getDatiSpecifici().values()) {
                ...
                valoreAttribFascicolo.setDecUsoModelloXsdFasc(
                        entityManager.find(DecUsoModelloXsdFasc.class, svf.getIdRecXsdUsoProfiloSpec()));
                valoreAttribFascicolo
                        .setDecAttribFascicolo(entityManager.find(DecAttribFascicolo.class, tmpDS.getIdDatoSpec()));
                valoreAttribFascicolo
                        .setDecAaTipoFascicolo(entityManager.find(DecAaTipoFascicolo.class, svf.getIdAATipoFasc()));
    ...
    private String generaXmlProfilo(Node profilo, boolean canonicalize) throws TransformerException {
        String xml = null;
    ...
    private String generaXmlProfiloSpec(JAXBElement<ProfiloSpecificoType> profile) throws JAXBException {
