...
public class ConfigurationDao implements IConfigurationDao {
    ...
    private String getParamApplicValue(String nmParamApplic, long idStrut, long idAmbiente, long idTipoUnitaDoc,
        ...
        String queryStrTempl = "SELECT NEW it.eng.parer.fascicolo.jpa.dto.AplVGetValParamDto (${" + APLVGETVALPARAMBYCOL
                + "}) " + "FROM AplParamApplic paramApplic JOIN ${" + APLVGETVALPARAMBY
                + "} getvalParam ON getvalParam.nmParamApplic = paramApplic.nmParamApplic "
                + "WHERE paramApplic.nmParamApplic = :nmParamApplic " + "AND paramApplic.${" + FLAPLPARAMAPPLICAPPART
                + "} = :flAppart ${" + IDAPLVGETVALPARAMBY + "} ";
