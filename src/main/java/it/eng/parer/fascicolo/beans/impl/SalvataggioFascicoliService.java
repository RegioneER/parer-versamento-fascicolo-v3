...
public class SalvataggioFascicoliService implements ISalvataggioFascicoliService {
    ...
    @Override
    @Transactional(value = TxType.REQUIRES_NEW, rollbackOn = { AppGenericPersistenceException.class })
    public void salvaFascicolo(RispostaWSFascicolo rispostaWs, VersFascicoloExt versamento,
        ...
        try {
            backendMetadata = objectStorageService.lookupBackendByServiceName(
                    versamento.getStrutturaComponenti().getIdAATipoFasc(), Costanti.WS_VERS_FASCICOLO_NOME);
            ...
            // salvo fascicolo
            rcSaveFasc = salvataggioFascicoliDao.scriviFascicolo(versamento, sessione, tmpFascicoloKo);
            ...
            salvataggioFascicoliDao.scriviRequestResponseFascicolo(rispostaWs, versamento, sessione, backendMetadata,
                    sipBlob, tmpFasFascicolo);
            ...
            salvataggioFascicoliDao.scriviProfiliXMLFascicolo(versamento, sessione, backendMetadata, profBlob,
                    tmpFasFascicolo);
            ...
            elencoVersamentoFascicoliService.scriviStatoConservFascicolo(rispostaWs, versamento, sessione,
                    tmpFasFascicolo);

            // salvo su elenco stato fascicolo
            elencoVersamentoFascicoliService.scriviStatoFascicoloElenco(rispostaWs, versamento, sessione,
                    tmpFasFascicolo);

            // rimuove fascicolo ko se necessario
            if (tmpFascicoloKo != null) {
                salvataggioFascicoliDao.ereditaVersamentiKoFascicolo(tmpFasFascicolo, tmpFascicoloKo);
            }
            //
            // MEV#30786
            /*
            ...
            if (backendMetadata.isObjectStorage()) {
                ...
                res = objectStorageService.createResourcesInMetaProfFascicoli(urn, backendMetadata.getBackendName(),
                        profBlob, tmpFasFascicolo.getIdFascicolo(), getIdStrut(versamento));
                log.debug("Salvati i METADATI di profilo nel bucket {} con chiave {} ", res.getBucket(), res.getKey());
            }
            // end MEV#30786
            entityManager.flush();
            ...
            rispostaWs.setEsitoWsErrBundle(MessaggiWSBundle.ERR_666P,
                    "Errore interno nella fase di salvataggio fascicolo: " + ExceptionUtils.getRootCauseMessage(ex));
            versamento.aggiungErroreFatale(rispostaWs.getCompRapportoVersFascicolo().getEsitoGenerale());
